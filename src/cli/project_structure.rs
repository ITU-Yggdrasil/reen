...
pub struct ProjectInfo {
    /// Folder -> direct module names (e.g., "contexts/ui" -> ["terminal_renderer"])
...
fn ensure_module_folder_hierarchy(folder: &str, project_info: &mut ProjectInfo) {
    ...
    for part in folder.split('/') {
        ...
        project_info.modules.entry(current.clone()).or_default();
    }
}
...
pub fn generate_mod_files(project_info: &ProjectInfo, output_dir: &Path) -> Result<()> {
    ...
    for folder in folders {
        ...
        let mut sorted_modules = project_info
            .modules
            .get(&folder)
            .cloned()
            .unwrap_or_default();
...
fn direct_child_subdirs(project_info: &ProjectInfo, folder: &str) -> Vec<String> {
    let prefix = format!("{}/", folder);
    let mut subdirs = BTreeSet::new();

    for candidate in project_info.modules.keys() {
...
mod tests {
    ...
    #[test]
    fn direct_child_subdirs_only_returns_one_level_down() {
        let mut project_info = ProjectInfo::default();
        project_info
            .modules
            .insert("contexts".to_string(), vec!["account".to_string()]);
        project_info.modules.insert(
            "contexts/ui".to_string(),
            vec!["terminal_renderer".to_string()],
        );
        project_info.modules.insert(
            "contexts/ui/widgets".to_string(),
            vec!["scoreboard".to_string()],
        );

        assert_eq!(direct_child_subdirs(&project_info, "contexts"), vec!["ui"]);
        assert_eq!(
            direct_child_subdirs(&project_info, "contexts/ui"),
            vec!["widgets"]
        );
    }

    #[test]
    fn ensure_module_folder_hierarchy_inserts_missing_parents() {
        let mut project_info = ProjectInfo::default();
        ensure_module_folder_hierarchy("contexts/ui/widgets", &mut project_info);
