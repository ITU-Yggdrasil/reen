...
**Description**: Executes the agent using the populated specification and the resolved model from the agent model registry. Invocation happens through the native Rust model runner. If the agent supports conversational style, the conversation should be treated as part of the template.
...
3. Execute the agent with the resolved model through the native Rust runner
...
- Native runner execution must be reliable
...
| `RUNNER_ERROR` | Native runner execution failed | "Failed to execute model runner: {{details}}" |
...
- Native Rust execution keeps provider integration inside the compiled binary
