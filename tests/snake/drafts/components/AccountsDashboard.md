...
AccountsDashboard is the primary authenticated overview surface for the Lupa account experience. It should be used to present a high-level financial summary and a list of visible accounts in one calm, clearly structured page shell.

---

## Visual Structure

AccountsDashboard is a dashboard layout with a dominant account-overview region that leads with a greeting and total-balance summary before exposing repeated account cards beneath it.

### Layout Structure

The layout should prioritize a clear vertical flow: summary first, account list second. On larger screens, the account region may use additional horizontal space, but the overall page should remain centered on the core financial overview.
...
- The summary region should visually dominate the page before the repeated account cards begin.
...
- **Default:** Standard authenticated dashboard with summary and accounts.
...
The page shows greeting, total balance, and visible account cards.

### Loading

Summary values and account cards may show placeholders while preserving the overall dashboard structure.
...
- The page should use semantic regions or sections to separate summary content and account content.
